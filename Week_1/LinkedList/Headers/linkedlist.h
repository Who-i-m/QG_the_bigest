 ...
 *	@description : Access the node and output the result(访问节点并输出数据)
 *	@param		 :e(LNode->data) 
 *	@return		 : void
 *  @notice      : None
 */
 ...
 *	@description : find the first node in the linked list according to e（根据e，找到链接列表中的第一个节点） 
 ...
 *	@description : reverse the nodes which value is an even number in the linked list, input: 1 -> 2 -> 3 -> 4  output: 2 -> 1 -> 4 -> 3（将链表中数值为偶数的节点倒过来，输入。1 -> 2 -> 3 -> 4 输出。2 -> 1 -> 4 -> 3） 
 *	@param		 : L(the head node)
